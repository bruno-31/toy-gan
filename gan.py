...
def train_gan():
    ...
    with tf.name_scope('generator_loss'):
        ...
        dis_logits_on_generated = tf.reduce_mean(dis_gen)
    with tf.name_scope('discriminator_loss'):
        d_loss_real = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=dis_data, labels=tf.ones_like(dis_data)))
        d_loss_fake = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=dis_gen, labels=tf.zeros_like(dis_gen)))
        d_loss = d_loss_fake + d_loss_real
        dis_logits_on_real = tf.reduce_mean(dis_data)
